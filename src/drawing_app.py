...
RECORDINGS_DIR = Path(__file__).resolve().parents[1] / "outputs" / "recordings"
...
class SkyScribbleApp:
    ...
    def _start_recording(self, w, h):
        ...
        path = str(RECORDINGS_DIR / f"skyscribble_{stamp}.mp4")
    ...
    def _draw_hud(self, output, mode_text):
        cv2.putText(output, f"Mode: {mode_text}", (10, 30),
            cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(output, "Keys: c=clear  r=rec  s=screenshot  q=quit", (10, 62),
            cv2.FONT_HERSHEY_SIMPLEX, 0.58, (230, 230, 230), 2, cv2.LINE_AA)
        if self._recording:
            cv2.circle(output, (output.shape[1] - 24, 24), 9, (0, 0, 255), -1)
            cv2.putText(output, "REC", (output.shape[1] - 60, 30),
        ...
        try:
            while True:
                ...
                if mode == "clear":
                    ...
                    cv2.line(canvas, (self.prev_x, self.prev_y), (x, y),
                ...
                if key == ord("q"):
                    ...
                    RECORDINGS_DIR.mkdir(parents=True, exist_ok=True)
                    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    path = str(RECORDINGS_DIR / f"screenshot_{stamp}.png")
                    cv2.imwrite(path, output)
                    print(f"[SNAP] Screenshot saved → {path}")
        finally:
            if self._recording:
                self._stop_recording()
            tracker.close()
