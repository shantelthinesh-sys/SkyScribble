...
const colors = ['#60a5fa','#f87171','#4ade80','#facc15','#a78bfa','#fb923c','#f9a8d4','#ffffff','#000000','#94a3b8']
...
const AnimeMode = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    ctx.strokeStyle = color; ctx.lineWidth = tool === 'erase' ? brushSize * 3 : brushSize
  ...
  const save = () => {
    const src = mode === 'air' ? undefined : canvasRef.current?.toDataURL()
    if (!src) return
    const a = document.createElement('a'); a.href = src; a.download = 'anime-colored.png'; a.click()
  }

  return (
    <Layout title="Anime Mode" subtitle="Upload a sketch and color it — use air drawing or mouse">
      <div className="grid lg:grid-cols-4 gap-6">
        <div className="space-y-4">
          ...
          <div className="bg-[#0F172A] border border-slate-800 rounded-xl p-4">
            <p className="text-xs text-slate-500 tracking-widest mb-3">UPLOAD SKETCH</p>
          ...
          {mode === 'mouse' && (
            <>
                ...
                <div className="flex gap-2">
                  {(['brush', 'erase'] as const).map(t => (
                    <button key={t} onClick={() => setTool(t)}
                      className={`flex-1 py-2 rounded-lg text-xs font-medium capitalize transition-all ${tool === t ? 'bg-blue-700 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>
                      {t}
                    </button>
                  ))}
                </div>
              ...
              <div className="bg-[#0F172A] border border-slate-800 rounded-xl p-4">
                ...
                <input type="range" min={2} max={30} value={brushSize} onChange={e => setBrushSize(+e.target.value)} className="w-full accent-blue-500" />
              </div>
            </>
          )}
          <div className="bg-[#0F172A] border border-slate-800 rounded-xl p-4">
          ...
          <button onClick={save} className="w-full py-3 bg-blue-700 hover:bg-blue-600 rounded-xl text-sm font-semibold text-white transition-colors">Download</button>
        </div>
        ...
        <div className="lg:col-span-3">
          {mode === 'air' ? (
            ...
            <div className="h-96 bg-[#0F172A] border border-dashed border-slate-700 rounded-2xl flex flex-col items-center justify-center gap-3">
              <p className="text-slate-600 text-sm">Upload an anime sketch to start coloring</p>
              <button onClick={() => setUploadedImg('blank')} className="px-4 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-300 hover:bg-slate-700 transition-colors">Blank Canvas</button>
            </div>
          ) : (
